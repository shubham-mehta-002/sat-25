...
import WitPost from './WitPost'

const WitPostPage = () => {
  const witid = useParams().witid
  const [WitPosts, setWitPosts] = useState([])
  const [witName, setWitName] = useState("")
  const [moderatorid, setModeratorid] = useState('')
  useEffect(() => {
    fetch(`${import.meta.env.VITE_BACKEND_URI}/api/getwitposts`, {
      method: 'POST',
      headers: {
      ...
      .then(result => {
        // setModeratorid(result[0].postedin.moderator);
        setWitPosts(result)
      ...
      <div className="profile text-center flex flex-col gap-8">
        <div className="post_bar">
          <div className='text-xl  flex justify-between px-3'>
            ...
            <NavLink to={`/wits/${witid}/createwitpost`} draggable="false">
              <button>
                Create Wit Post
              </button>
            </NavLink>
